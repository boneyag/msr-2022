...
import re
from regex_patterns import REGEX_LIST
...
def get_sentence_text_no_lem(sentence):
    res = ""
    for word in sentence['tokens']:
        res += word['word']+ " "
...
def get_all_paragraph_sentences2(paragraph):
	corenlp_properties={'annotators': 'pos,parse,lemma', 'outputFormat': 'json'}
	annotations = corenlp.annotate(paragraph, corenlp_properties)
	all_sentences = list()
	...
	try:
		for sentence in annotations['sentences']:
			sentence_text = get_sentence_text_no_lem(sentence)
			all_sentences.append(sentence_text)
	except:
		print("Failed to process paragraph:" + paragraph, file=sys.stderr)
